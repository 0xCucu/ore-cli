...
async fn main() {
    ...
    let exec_cluster = args.exec_rpc.unwrap_or(cluster.clone());
    ...
    let miner = Arc::new(Miner::new(
        cluster.clone(),
        args.priority_fee,
        Some(default_keypair.clone()),
        client,
        exec_client
    ));
    println!("exec_cluster {}, read_cluster {} , wallet {} {}",exec_cluster,cluster,miner.signer().pubkey(),&default_keypair);
